package com.yb.cloudPictrueBackend.service.impl;
...
import com.yb.cloudPictrueBackend.manager.CosManager;
import com.yb.cloudPictrueBackend.manager.FileManager;
import com.yb.cloudPictrueBackend.manager.upload.FilePictureUpload;
import com.yb.cloudPictrueBackend.manager.upload.PictureUploadTemplate;
import com.yb.cloudPictrueBackend.manager.upload.UrlPictureUpload;
import com.yb.cloudPictrueBackend.mapper.PictureMapper;
...
import com.yb.cloudPictrueBackend.model.enums.PictureReviewStatusEnum;
import com.yb.cloudPictrueBackend.model.vo.PictureVO;
import com.yb.cloudPictrueBackend.model.vo.UserVO;
import com.yb.cloudPictrueBackend.service.PictureService;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        //上传图片,得到返回的图片信息
        //按照用户id划分目录
        String uploadPathPrefix = String.format("pulic/%s", loginUser.getId());
        ...
        //如果pictureId不为空，则代表是更新，否则是新增
        if (pictureId != null) {
            //如果是更新，加上基础信息，补充上id和编辑时间
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "抓取数量最多为30条");
        //要抓取的地址
    ...
    @Override
    public void deleteCos(DeleteRequest deleteRequest) {
        Picture picture =  baseMapper.selectById(deleteRequest.getId());
        String thumbnailUrl = picture.getThumbnailUrl();
        String pictureUrl = picture.getUrl();
        // 提取文件名
        String thumbnailFileName = extractFileNameFromUrl(thumbnailUrl);
        String pictureFileName = extractFileNameFromUrl(pictureUrl);
        if (StrUtil.isNotBlank(pictureFileName)){
            cosManager.deleteObject(pictureFileName);
        }
        if (StrUtil.isNotBlank(thumbnailFileName)){
            cosManager.deleteObject(thumbnailFileName);
        }
    }
    // 提取 URL 中的文件名
    private String extractFileNameFromUrl(String url) {
        ...
        return url.substring(url.indexOf("pulic/"));
    }
}
